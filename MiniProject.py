...
def barchartfunction(mainDataFrame, groups, subgroups, num, xaxis,title):
    ...
    plt.xticks([r2 + barWidth for r2 in range(num)], groups, rotation=45)
...
completionSector2 = df.groupby(['sector', 'implementation_status']).sum().total_amount.unstack().plot.bar(stacked=True)
plt.title('Sector Expenditure by Implementation Status', fontsize=16, fontweight='bold')
...
countySector2 = df.groupby(['county', 'implementation_status']).sum().total_amount.unstack().plot.bar(stacked=True)
plt.title('County Expenditure by Implementation Status', fontsize=16, fontweight='bold')

# No of Projects per County
plt.figure()
df.groupby('county')['objectid'].count().sort_values(ascending=False).plot.bar()
plt.title('No. of Projects Per County', fontsize=16, fontweight='bold')

# No of Projects per Sector
plt.figure()
df.groupby('sector')['objectid'].count().sort_values(ascending=False).plot.bar()
plt.title('No. of Projects Per Sector', fontsize=16, fontweight='bold')

# Estimated Output/ Actual Totals per Sector
df.groupby('sector')['estimated_cost', 'total_amount'].sum().sort_values('estimated_cost', ascending=False).plot.bar()
plt.title('Estimates Output vs Actual Totals per Sector', fontsize=16, fontweight='bold')

# Estimated Output/ Actual Totals per County
df.groupby('county')['estimated_cost', 'total_amount'].sum().sort_values('total_amount', ascending=False).plot.bar()
plt.title('Estimates Output vs Actual Totals per County', fontsize=16, fontweight='bold')